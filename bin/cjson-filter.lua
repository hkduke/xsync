...
local function create_config_cjson ()
    -- 首先从全局配置中加载 cjson 模块
    local libcjson = _G["cjson"]

    -- 如果不存在则创建
    if not libcjson then
        print("WARN: 'cjson' not found in _G table !")
        libcjson = require("cjson")
    end

    if not libcjson then
        print("ERROR: fail to load cjson.so !")
        return nil
    end

    print("SUCCESS: loaded cjson.so")

    -- 创建一个副本 (默认配置)
    local cjson2 = libcjson.new()
