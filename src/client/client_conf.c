 ...
 * 必须在锁 event_lock 中调用
 */
int XS_client_prepare_watch_events (XS_client client, struct inotify_event *inevent, XS_watch_event events[XSYNC_SERVER_MAXID + 1])
{
    ...
    int hash = BKDRHash2(inevent->name, XSYNC_HASHMAP_MAX_LEN);

    hlist_for_each(hp, &client->event_hmap[hash]) {
        struct xs_watch_event_t *p = hlist_entry(hp, struct xs_watch_event_t, i_hash);

        if (! strncmp(p->pathname, inevent->name, inevent->len)) {
            // LOGGER_WARN("found event(=%p): task=%lld name=%s", p, (long long) p->taskid, p->pathname);
            return 0;
        }
    }

    XS_watch_event_create(inevent, client, 0, &event);

    LOGGER_DEBUG("create event(=%p): task=%lld name=%s", event, (long long) event->taskid, event->pathname);
